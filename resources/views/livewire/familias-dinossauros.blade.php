<div
    ...
    x-data="{
        showNovaFamilia: @entangle('showNovaFamilia'),
        showSuccess: @entangle('showSuccess'),
    }"
>
    <p class="p-9 mb-6 text-2xl font-bold text-yellow-900 underline">Famílias</p>

    <div class="px-8">
        <x-button
            class="text-white bg-lime-700 hover:bg-lime-800"
            title="Novo"
            x-on:click="showNovaFamilia = true"
        >
        ...
        <x-modal class="bg-yellow-800" trigger="showNovaFamilia">
            <p class="text-5xl font-extrabold text-center text-white">
                Nova família de dinossauro
            </p>
            <form
                class="flex flex-col items-center p-24"
                wire:submit.prevent="saveFamiliaDinossauro"
            >
            <x-input
                class="px-5 py-3 border border-lime-400 w-80"
                type="text"
                name="no_familia_dinossauro"
                placeholder="Ex.: Estegossauros"
                wire:model.defer="no_familia_dinossauro"
                required
            >
            </x-input>

            <span class="text-xs text-gray-100">
                {{ $errors->has('no_familia_dinossauro') }}
            </span>

            <x-button class="justify-center px-5 py-3 mt-5 bg-lime-600 w-80 hover:bg-lime-700">
                <span class="animate-spin" wire:loading wire:target='saveFamiliaDinossauro'>&#9696;</span>
                <span wire:loading.remove wire:target='saveFamiliaDinossauro'>Salvar</span>
            </x-button>
        </form>
        </x-modal>
        <x-input
